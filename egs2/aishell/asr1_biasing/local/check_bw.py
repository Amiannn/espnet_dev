...
blist    = "./local/rareword.train.txt"
# blist    = "./local/rareword.all.sep.txt"
bpemodel = "./data/zh_token_list/bpe_unigram4500suffix/bpe.model"
charlist = "./data/zh_token_list/bpe_unigram4500suffix/tokens.txt"

ref_path     = "./data/zh_test/text"
utt_ent_path = "./local/test.bio.utt.json"

ref_datas  = [[d[0], ' '.join(d[1:])] for d in read_file(ref_path, sp=' ')]
utt_entity = read_json(utt_ent_path)
...
print(bwords[:10])
output_path = os.path.join('./local', 'rareword.train.sep.txt')
