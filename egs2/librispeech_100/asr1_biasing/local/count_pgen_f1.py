...
if __name__ == '__main__':
    ...
    for data in datas:
        ...
        for pred in prediction:
            ...
            hyp_ent  = " ".join(hyp_ent)
            biasword = biasword.replace("_", " ") 

            pgen = pgens[start:end]
            print(f'biasword: {biasword}')
            print(f'hyp_ent : {hyp_ent}')
            print(f'pgen    : {pgen}')
            TP, FN, FP, TN = check_right(biasword, hyp_ent, pgen)
            ...
            utt_fp += FP
            utt_tn += TN
        utt_tp /= len(prediction)
        utt_fn /= len(prediction)
        utt_fp /= len(prediction)
        utt_tn /= len(prediction)
        avg_tp += utt_tp
        avg_fn += utt_fn
        avg_fp += utt_fp
        avg_tn += utt_tn
    ...
    print(f'AVG      : TP: {TP:.2f}, FN: {FN:.2f}, FP: {FP:.2f}, TN: {TN:.2f}')
