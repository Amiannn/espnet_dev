...
if __name__ == "__main__":
    ...
    word = "C"
    tokens = model.bprocessor.tokenizer.text2tokens(word)
    print(f'tokens: {tokens}')
    idxs   = [model.bprocessor.chardict[token] for token in tokens]
    print(f'idxs: {idxs}')

    idxs = torch.tensor([5, 7, 0, 0])
    ...
    print(f'embeds:\n{embeds[idxs]}')
    print(f'embeds shape: {embeds[idxs].shape}')
