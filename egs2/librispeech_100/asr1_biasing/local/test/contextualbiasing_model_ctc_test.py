...
config = {
    ...
    "asr_model_file": "exp/asr_finetune_freeze_conformer_transducer_contextual_biasing_sche30_rep_suffix/valid.loss.ave_10best.pth",
...
def model_forward(
    ...
    **kwargs,
    ...
    # partial output
    lin_enc_out  = model.joint_network.lin_enc(encoder_out)
    act_enc_out  = model.joint_network.joint_activation(lin_enc_out)
    lin_enc_out  = model.joint_network.lin_out(act_enc_out)
    logp_enc_out = torch.log_softmax(
        lin_enc_out,
        dim=-1,
    )[0]
    print(f'logp_enc_out shape: {logp_enc_out.shape}')

    enc_pred = torch.argsort(logp_enc_out, dim=-1, descending=True)[:, :100]
    print(f'enc_pred shape: {enc_pred.shape}')
    frame_outputs = []
    for t in range(enc_pred.shape[0]):
        frame_out = [model.token_list[idx] for idx in enc_pred[t]]
        frame_outputs.append(frame_out)

    output_path = os.path.join(debug_path, f'{uttid}_frameout.tsv')
    write_file(output_path, frame_outputs, sp='\t')

    enc_pred_mean = torch.mean(logp_enc_out, dim=0)
    output_path = os.path.join(debug_path, f'{uttid}_frameout_mean.pickle')
    write_pickle(output_path, enc_pred_mean)
    ...
    # partial output
    lin_dec_out  = model.joint_network.lin_dec(decoder_out)
    act_dec_out  = model.joint_network.joint_activation(lin_dec_out)
    lin_dec_out  = model.joint_network.lin_out(act_dec_out)
    logp_dec_out = torch.log_softmax(
        lin_dec_out,
        dim=-1,
    )[0]
    print(f'logp_dec_out shape: {logp_dec_out.shape}')

    dec_pred = torch.argsort(logp_dec_out, dim=-1, descending=True)[:, :100]
    print(f'dec_pred shape: {dec_pred.shape}')
    label_outputs = []
    for u in range(dec_pred.shape[0]):
        label_out = [model.token_list[idx] for idx in dec_pred[u]]
        label_outputs.append(label_out)

    output_path = os.path.join(debug_path, f'{uttid}_labelout.tsv')
    write_file(output_path, label_outputs, sp='\t')

    dec_pred_mean = torch.mean(logp_dec_out, dim=0)
    output_path = os.path.join(debug_path, f'{uttid}_labelout_mean.pickle')
    write_pickle(output_path, dec_pred_mean)
