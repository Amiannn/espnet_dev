...
def model_forward(
    ...
    **kwargs,
    ...
    labels       = [i for i in range(len(worddict) + len(biasingwords))]
    ...
    realbwords = []
    for i in range(cb_tokens.shape[0]):
        cb_tokens_embed = embed_matrix[cb_tokens[i, :]]
    ...
    for i in range(enc.shape[0]):
        ...
        # frame2bsub = torch.stack(frame2bsub + realbwords).squeeze(1)
        frame2bsub = torch.stack(bembed + realbwords).squeeze(1)
        ...
        frame2blist.append(", ".join(frame + [f'{k}. {biasingwords[k]}' for k in range(len(biasingwords))]))
