...
config = {
    "log_level": "INFO",
    "output_dir": "exp/asr_finetune_freeze_conformer_transducer_contextual_biasing_predictor_proj_suffix/decode_asr_asr_model_valid.loss.best/test_clean/logdir/output.1",
    "ngpu": 0,
    "seed": 0,
    "dtype": "float32",
    "num_workers": 1,
    ...
    "key_file": "exp/asr_finetune_freeze_conformer_transducer_contextual_biasing_predictor_proj_suffix/decode_asr_asr_model_valid.loss.best/test_clean/logdir/keys.1.scp",
    "allow_variable_data_keys": False,
    "asr_train_config": "exp/asr_finetune_freeze_conformer_transducer_contextual_biasing_predictor_proj_suffix/config.yaml",
    "asr_model_file": "exp/asr_finetune_freeze_conformer_transducer_contextual_biasing_predictor_proj_suffix/valid.loss.best.pth",
...
def model_forward(
    ...
    **kwargs,
    ...
    sem_bias, sem_atten = model.get_semantic_biasing_vector(decoder_out, cb_embed, return_atten=True)
    print(f'sem_atten shape: {sem_atten.shape}')
    ...
    #     speech.to('cpu')[0]
    # )
    sem_atten   = sem_atten.squeeze(0).T.detach().cpu().resolve_conj().resolve_neg().numpy()
    # frame2align = {start: token for token, start, end in alignments}
    # xlabels = [frame2align[i] if i in frame2align else '' for i in range(sem_atten.shape[1])]
    print(f'text: {text}')
    xlabels = ['<blank>'] + [model.token_list[t] for t in text[0]]
    print(f'xlabels: {len(xlabels)}')

    # draw attention map
    fig, axes = plt.subplots(1, 1, figsize=(30, 10))
    axes.xaxis.set_ticks(np.arange(0, sem_atten.shape[1], 1))
    axes.yaxis.set_ticks(np.arange(0, sem_atten.shape[0], 1))
    axes.set_xticks(np.arange(-.5, sem_atten.shape[1], 1), minor=True)
    axes.set_yticks(np.arange(-.5, sem_atten.shape[0], 1), minor=True)
    axes.set_xticklabels(xlabels)
    axes.set_yticklabels(labels)

    axes.imshow(sem_atten, aspect='auto')
    axes.grid(which='minor', color='w', linewidth=0.5, alpha=0.3)
    plt.title(ref_text)
    output_path = os.path.join(debug_path, f'{uttid}_BA_predictor.pdf')
