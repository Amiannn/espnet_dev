...
config = {
    "log_level": "INFO",
    "output_dir": "exp/asr_finetune_freeze_conformer_transducer_contextual_biasing_proj_encode_suffix/decode_asr_asr_model_valid.loss.ave_10best/test_clean/logdir/output.1",
    "ngpu": 0,
    "seed": 0,
    "dtype": "float32",
    "num_workers": 1,
    ...
    "key_file": "exp/asr_finetune_freeze_conformer_transducer_contextual_biasing_proj_encode_suffix/decode_asr_asr_model_valid.loss.ave_10best/test_clean/logdir/keys.1.scp",
    "allow_variable_data_keys": False,
    "asr_train_config": "exp/asr_finetune_freeze_conformer_transducer_contextual_biasing_proj_encode_suffix/config.yaml",
    "asr_model_file": "exp/asr_finetune_freeze_conformer_transducer_contextual_biasing_proj_encode_suffix/valid.loss.ave_10best.pth",
...
def model_forward(
    ...
    **kwargs,
    ...
    enc_embeds = model.Qproj_acoustic(encoder_out).squeeze(0)
    enc_embeds = enc_embeds / torch.norm(enc_embeds, dim=0)
    # enc_embeds = torch.nn.functional.normalize(enc_embeds)
    print(f'enc_embeds: {enc_embeds.shape}')

    labels       = [''.join(key).replace('▁', '') for key in worddict] + ['<OOL>']
    # labels       = [''.join(key).replace('▁', '') for key in worddict]
    ...
    input_sizes = input_sizes.to(encoder_out.device)
    cb_embed    = torch.sum(cb_seq_embed_packed, dim=1) / input_sizes.unsqueeze(-1)
    # cb_embed    = torch.cat([cb_embed, embed_matrix[-1].unsqueeze(0)], dim=0)
    print(f'cb_embed: {cb_embed.shape}')
    tsne = TSNE(n_components=2, verbose=1, perplexity=40, n_iter=300)

    _cb_embed = model.Kproj(cb_embed)
    _cb_embed = _cb_embed / torch.norm(_cb_embed, dim=0)

    # plot scatter
    cb_class = [1 if word in biasingwords else 2 for word in labels]
    plt.rcParams.update({'font.size': 3})
    X = torch.cat([enc_embeds, _cb_embed], dim=0)
    C = torch.cat([torch.zeros(enc_embeds.shape[0]), torch.tensor(cb_class)], dim=0).to(torch.int)
    print(f'C: {C}') 
    print(f'C: {C.shape}') 
    Color  = ["#4C72B0", "#FFA500", "#808080", "#008000", "#FF0000"]
    Shape  = ['o', '^', '.', 'x']
    ...
    for i in range(_cb_embed.shape[0]):
        x, y = X[start + i, 0], X[start + i, 1]
        texts.append(plt.text(x, y, labels[i]))
    adjust_text(texts)
    output_path = os.path.join(debug_path, f'{uttid}_proj.pdf')
    ...
    xlabels = [f'{frame2align[i]} {i}' if i in frame2align else f'{i}' for i in range(aco_atten.shape[1])]
