...
def model_forward(
    ...
    **kwargs,
    ...
    lin_enc_out = model.joint_network.lin_enc(encoder_out.unsqueeze(2))
    lin_dec_out = model.joint_network.lin_dec(decoder_out.unsqueeze(1))

    lin_mean_enc_out = torch.mean(lin_enc_out, dim=1).reshape(-1)
    lin_mean_dec_out = torch.mean(lin_dec_out, dim=2).reshape(-1)
    
    print(f'lin_mean_enc_out norm: {torch.norm(lin_mean_enc_out)}')
    print(f'lin_mean_dec_out: {torch.norm(lin_mean_dec_out)}')

    joint_out = model.joint_network.joint_activation(
        lin_enc_out + lin_dec_out
    )
