...
config = {
    ...
    "asr_model_file": "exp/asr_finetune_freeze_conformer_transducer_contextual_biasing_proj_suffix/valid.loss.best.pth",
...
def model_forward(
    ...
    **kwargs,
    ...
    labels       = [token for token in model.token_list] + ['<OOL>']
    ...
    B, N, D = cb_seq_embed.shape
    V       = model.vocab_size
    # B: biasing length
    # N: bias subword seq length
    # D: hidden dim
    # V: vocab size

    device  = encoder_out.device
    index   = cb_tokens.reshape(B * N)
    source  = cb_seq_embed.reshape(B * N, D)
    
    _cb_embed = torch.zeros(V + 1, D).to(device)
    _cb_embed.index_add_(0, index, source)
    
    _cb_length = torch.zeros(V + 1).to(device)
    _cb_length.index_add_(0, index, torch.ones(B * N).to(device))
    _cb_length[0] = 0
    _cb_length    = _cb_length.unsqueeze(1)
    print(f'cb length: {_cb_length}')
    index, _       = torch.where(_cb_length!=0)
    _cb_embed      = _cb_embed / _cb_length
    cb_key_embed   = _cb_embed[index, :]
    cb_value_embed = embed_matrix[index, :]
    ...
    plt.title(ref_text)
    output_path = os.path.join(debug_path, f'{uttid}_BA_vocab.pdf')
