...
class ESPnetTCPGenBiasingKLLossASRModel(ESPnetASRModel):
    ...
    def __init__(
        ...
        if biasinglist != "":
            ...
            logging.info(f'confusion table path -> {confusion_table_path}')
            confusion_table = read_numpy(confusion_table_path)
            # ookb
            self.confusion_table = torch.from_numpy(confusion_table)
            # self.confusion_table = torch.nn.functional.softmax(self.confusion_table, dim=-1)

            logging.info(f'confusion table -> {self.confusion_table}')
            logging.info(f'confusion table [10] -> {self.confusion_table[10]}')
            logging.info(f'confusion table shape -> {self.confusion_table.shape}')
        ...
        if self.use_transducer_decoder:
            ...
            if self.biasing:
                ...
                self.criterion_klloss = torch.nn.KLDivLoss(reduction="batchmean")
        ...
        **kwargs,
        ...
        if self.use_transducer_decoder:
            ...
            if loss_ctc is not None:
                loss = loss_transducer + (self.ctc_weight * loss_ctc) + (kl_weight * klloss)
            else:
                loss = loss_transducer + (kl_weight * klloss)

            # Collect Transducer branch stats
            stats["loss_transducer"] = (
                loss_transducer.detach() if loss_transducer is not None else None
            )
            stats["loss_kl_pho_confusion"] = (
                klloss.detach() if klloss is not None else None
            )
    ...
    def _calc_transducer_loss(
        ...
        if self.biasing and self.epoch >= self.biasingsche:
            ...
            # logging.info(f'decoder_in shape: {decoder_in.shape}')
            # logging.info(f'target shape: {target.shape}')
            # logging.info(f'ptr_dist shape: {ptr_dist.shape}')

            # klloss
            ptr_dist_fact = ptr_dist[:, :, :-1, :-1]
            ptr_dist_ool  = ptr_dist[:, :, :-1, -1:]
            trie_mask  = (ptr_dist_fact == 0)[:, :, :, :]
            
            ptr_dist_fact_log = torch.nn.functional.log_softmax(ptr_dist_fact, dim=-1)

            B, T, U, V = ptr_dist_fact.shape
            confusion_target = self.confusion_table[target.long(), :]
            confusion_target = confusion_target.unsqueeze(1).repeat(1, T, 1, 1)
            confusion_target = to_device(self, confusion_target)

            confusion_target.masked_fill_(trie_mask, -1e9)
            confusion_target = torch.nn.functional.softmax(confusion_target, dim=-1)

            # logging.info(f'confusion_target : {confusion_target[0, 0, 0, :]}')
            # logging.info(f'ptr_dist_fact_log: {ptr_dist_fact_log[0, 0, 0, :]}')

            klloss = self.criterion_klloss(ptr_dist_fact_log, confusion_target) / (T * U)
            # logging.info(f'klloss: {klloss}')
        ...
        return loss_transducer, klloss, cer_transducer, wer_transducer
    ...
    def get_meetingKB_emb_map(
        ...
        mask     = meeting_mask.bool().unsqueeze(1).repeat(1, query.size(1), 1)
        KBweight.masked_fill_(mask, -1e9)
